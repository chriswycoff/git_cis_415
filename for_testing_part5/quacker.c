...
volatile int number_of_file_issued_commands = 0; 
/*-----------------define command keys----------------------------------------------------*/
...
char top_of_html[200] = "<!DOCTYPE html><html><head><meta http-equiv=\"refresh\" content=\"3\">";

char style_of_html[300] = "<style>table, th, td {border: 1px solid black;border-collapse: collapse;}th,td{padding: 5px;}th{text-align: left;}</style></head><body>";

char h1_html_1[300] = "<h1>Subscriber: ";
...
pthread_mutex_t command_num_mutex = PTHREAD_MUTEX_INITIALIZER;
...
void handle_subscriber(char* command_file, struct threadargs* my_arguments){
	...
	*/
	...
	int got_topics_ids[200];
	char got_topics[200][200];
	char got_urls[200][200];
	...
	fputs("\n", htmlfp);
	fputs("\n", htmlfp);
	strcpy(src, command_file);
	strcpy(dest, h1_html_1);
	strcat(dest, src);
	...
	//char h1_html_1[300] = "<h1>Subsriber:";
	//char h1_html_2[300] = "</h1>";
...
while(continue_parsing){
	...
	if(line_buffer[0] == 'c'){
	sscanf(line_buffer, "%s %s %d %s %d", tokens[0], tokens[1], &digit_1 ,tokens[3], &digit_2);
	}

	if(line_buffer[0] == 'q'){
	sscanf(line_buffer, "%s %s", tokens[0], tokens[1]);
	}

	if(line_buffer[4] == 'p'){
	sscanf(line_buffer, "%s %s %s", tokens[0], tokens[1], tokens[2]);
	}
	...
	if(line_buffer[0] == 'd'){
	sscanf(line_buffer, "%s %d", tokens[0], &digit_1);
	}

	if(line_buffer[0] == 's'){
	sscanf(line_buffer, "%s", tokens[0]);
	}
...
////////////////////// End File Parsing for main command file ///////////////////////
	...
	*/
	...
	printf("commands_left: %d\n", number_of_file_issued_commands );
	free(original_line);
	exit_function();
}
////// END MAIN /////////////////////////////////////////////////////
